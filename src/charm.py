...
class CharmOpenidcTestFixtureCharm(CharmBase):
    ...
    def _on_install(self, event):
        ...
        container = client.containers.run(
            client.images.list()[0].id,
            detach=True, ports={"8443": "443"},
            environment={"KEYCLOAK_ADMIN": "admin",
                         "KEYCLOAK_ADMIN_PASSWORD": "admin"}
        )
        subprocess.check_call("./src/wait_for_tcp_port.sh", shell=True)
        container.exec_run(cmd=('/opt/keycloak/bin/kcadm.sh create '
        ...
        container.exec_run(cmd=('/opt/keycloak/bin/kcadm.sh create '
                                ...
                                '-s username=johndoe -s enabled=true '
                                '-r demorealm'))
        container.exec_run(cmd=('/opt/keycloak/bin/kcadm.sh create '
                                '--server http://localhost:8080 '
                                '--realm master  --user admin '
                                '--password admin users '
                                '-s username=janedoe -s enabled=true '
                                '-r demorealm'))
        container.exec_run(cmd=('/opt/keycloak/bin/kcadm.sh set-password '
                                '--server http://localhost:8080 '
                                '--realm master  --user admin '
                                '--password admin   --username janedoe '
                                '-r demorealm --new-password crapper'))
        container.exec_run(cmd=('/opt/keycloak/bin/kcadm.sh set-password '
                                '--server http://localhost:8080 '
                                '--realm master  --user admin '
                                '--password admin   --username johndoe '
                                '-r demorealm --new-password crapper'))
